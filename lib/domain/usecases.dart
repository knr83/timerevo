...
class ClockInUseCase {
  ...
  Future<ClockActionResult> call(int employeeId) async {
    ...
    if (employee == null) return const ClockError(ClockErrorKind.employeeInactive);
    if (!employee.isActive) return const ClockError(ClockErrorKind.employeeInactive);
    final open = await _sessionsRepo.getOpenSessionForEmployee(employeeId);
    if (open != null) return const ClockError(ClockErrorKind.sessionAlreadyOpen);
    final today = todayYmd();
    final hasAbsence = await _absencesRepo.hasApprovedAbsenceOnDate(employeeId, today);
...
class EmployeeReportWithNormUseCase {
  ...
  Stream<List<EmployeeReportRowInfo>> streamEmployeeReportWithNorm({
    ...
    await for (final rows
        in _sessionsRepo.streamEmployeeReport(
          fromUtcMs: fromUtcMs,
          toUtcMs: toUtcMs,
        )) {
  ...
  Future<List<EmployeeReportRowInfo>> _enrichWithNorm(
    ...
    for (final r in rows) {
      final ranges = await _absencesRepo.getApprovedAbsenceRangesForEmployeeInPeriod(
        r.employeeId,
        dateToYmd(fromDate),
        dateToYmd(toDate),
      );
      ...
      for (final range in ranges) {
        var d = DateTime(range.dateFrom.year, range.dateFrom.month, range.dateFrom.day);
        final end = DateTime(range.dateTo.year, range.dateTo.month, range.dateTo.day);
        while (!d.isAfter(end)) {
          set.add(dateToYmd(d));
...
class EmployeeDayReportUseCase {
  ...
  Stream<List<EmployeeDayReportRow>> streamEmployeeDayReport({
    ...
    await for (final sessions
        in _sessionsRepo.streamSessions(
          employeeId: employeeId,
          fromUtcMs: fromUtcMs,
          toUtcMs: toUtcMs,
        )) {
      ...
      for (final s in sessions) {
        ...
        final startLocal = DateTime.fromMillisecondsSinceEpoch(s.startTs, isUtc: true).toLocal();
      ...
      while (!d.isAfter(toDate)) {
        ...
        final dayMin = ReportNormCalculator.dayNormMinutes(schedule, hasAbsence);
...
class EmployeesAdminUseCase {
  ...
  Future<bool> checkCodeUnique(String code, {int? excludeEmployeeId}) =>
      _employeesRepo.checkCodeUnique(code, excludeEmployeeId: excludeEmployeeId);
...
class SchedulesTemplatesUseCase {
  ...
  Future<void> saveTemplateWeek({
    ...
    required Map<int, DaySchedule> days,
      ...
      _schedulesRepo.saveTemplateWeek(
        templateId: templateId,
        days: days,
      );
...
class ChangeAdminPinUseCase {
  ...
  Future<bool> changeAdminPin({
    ...
    return _authRepo.changeAdminPin(
