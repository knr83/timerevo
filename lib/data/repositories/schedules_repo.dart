...
class SchedulesRepo implements ISchedulesRepo {
  ...
  Future<int?> getAssignmentTemplateId(int employeeId) async {
    final a = await (_db.select(_db.employeeScheduleAssignments)
  ...
  Future<int> createTemplate(String name) async {
    return guardRepoCall(() async {
      return _db.transaction(() async {
        ...
        final id = await _db.into(_db.shiftScheduleTemplates).insert(
  ...
  Future<void> updateTemplateName({required int id, required String name}) async {
    return guardRepoCall(() async {
  ...
  Future<void> setTemplateActive({required int id, required bool isActive}) async {
    return guardRepoCall(() async {
      await (_db.update(_db.shiftScheduleTemplates)
            ..where((t) => t.id.equals(id)))
          .write(
  ...
  Future<bool> hasAssignments(int templateId) async {
    final row = await (_db.select(_db.employeeScheduleAssignments)
  ...
  Future<void> deleteTemplate(int templateId) async {
    return guardRepoCall(() async {
      ...
      await _db.transaction(() async {
        ...
        final days = await (_db.select(_db.shiftScheduleTemplateDays)
              ..where((d) => d.templateId.equals(templateId)))
            .get();

        for (final day in days) {
          await (_db.delete(_db.shiftScheduleTemplateIntervals)
                ..where((i) => i.templateDayId.equals(day.id)))
              .go();
        }

        await (_db.delete(_db.shiftScheduleTemplateDays)
              ..where((d) => d.templateId.equals(templateId)))
            .go();

        await (_db.delete(_db.shiftScheduleTemplates)
  ...
  Stream<Map<int, DaySchedule>> watchTemplateWeek(int templateId) {
    ...
    final join = _db.select(d).join([
      leftOuterJoin(i, i.templateDayId.equalsExp(d.id)),
    ])
      ..where(d.templateId.equals(templateId))
      ..orderBy([OrderingTerm.asc(d.weekday), OrderingTerm.asc(i.startMin)]);
    ...
    return join.watch().map((rows) {
      ...
      for (final r in rows) {
        ...
        if (intervalRow != null &&
            intervalRow.endMin > intervalRow.startMin) {
  ...
  Future<void> saveTemplateDay({
    ...
    return guardRepoCall(() async {
      ...
      final day = await (_db.select(_db.shiftScheduleTemplateDays)
            ..where((d) =>
                d.templateId.equals(templateId) & d.weekday.equals(weekday))
            ..limit(1))
          .getSingleOrNull();
      if (day == null) return;

      await _db.transaction(() async {
        await (_db.update(_db.shiftScheduleTemplateDays)
              ..where((d) => d.id.equals(day.id)))
            .write(
  ...
  Future<void> saveTemplateWeek({
    ...
    return guardRepoCall(() async {
      ...
      await _db.transaction(() async {
        final dayRows = await (_db.select(_db.shiftScheduleTemplateDays)
              ..where((d) => d.templateId.equals(templateId)))
            .get();

        for (final dayRow in dayRows) {
          final wd = dayRow.weekday;
          final schedule = days[wd] ??
              const DaySchedule(isDayOff: true, intervals: []);

          await (_db.update(_db.shiftScheduleTemplateDays)
                ..where((d) => d.id.equals(dayRow.id)))
              .write(ShiftScheduleTemplateDaysCompanion(
                isDayOff: Value(schedule.isDayOff ? 1 : 0),
              ));

          await (_db.delete(_db.shiftScheduleTemplateIntervals)
                ..where((i) => i.templateDayId.equals(dayRow.id)))
              .go();
  ...
  Future<int> createTemplateWithWeek({
    ...
    return guardRepoCall(() async {
      ...
      return _db.transaction(() async {
        ...
        final insertedDays = await (_db.select(_db.shiftScheduleTemplateDays)
  ...
  Future<ResolvedSchedule> resolveSchedule({
    ...
    final assignment = await (_db.select(_db.employeeScheduleAssignments)
        ...
        .getSingleOrNull();
    if (assignment == null) {
      return const ResolvedSchedule(source: 'none', isDayOff: false, intervals: []);
    }
    ...
    final intervals = await (_db.select(_db.shiftScheduleTemplateIntervals)
          ..where((i) => i.templateDayId.equals(day.id))
